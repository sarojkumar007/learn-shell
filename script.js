...
function changePage(pg) {
  if (pg < 1) pg = 1
  if (pg > allSlides.length) pg = allSlides.length
...
window.onload = function () {
  ...
  // initial
  const currPage = parseInt(new URLSearchParams(location.search).get('page')) || 1
  changePage(currPage)
}
...
window.addEventListener('keyup', e => {
  ...
  if (['ArrowUp', 'ArrowLeft'].includes(e.key) && currPage > 1) {
    // console.log(`Prev Slide`)
    changePage(currPage - 1)
  }
  if (['ArrowDown', 'ArrowRight'].includes(e.key) && currPage < allSlides.length) {
    // console.log(`Next Slide`)
    changePage(currPage + 1)
  }
})

document.querySelector('#nav_prev').addEventListener('click', () => {
  const currPage = parseInt(new URLSearchParams(location.search).get('page')) || 1
  if (currPage > 1) changePage(currPage - 1)
})

document.querySelector('#nav_next').addEventListener('click', () => {
  const currPage = parseInt(new URLSearchParams(location.search).get('page')) || 1
  if (currPage < allSlides.length) changePage(currPage + 1)
})